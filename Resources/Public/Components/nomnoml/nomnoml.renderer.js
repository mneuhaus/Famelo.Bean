...
nomnoml.render = function (graphics, config, compartment, setFont){
	...
	function renderCompartment(compartment, style, level){
		...
		_.each(compartment.lines, function (text, i){
			...
			if (style.center != true) {
				...
				g.path([{x:x - 9, y:y + (config.leading * 4)}, {x:x + compartment.width - 9, y:y + (config.leading * 4)}]).stroke()
			}
		})
	...
	function findPropertyCoordinates(node, name) {
		...
		for (var i = 0; i < compartment.lines.length; i++) {
			...
			if (propertyName == name) {
				return {
					x: node.x + (compartment.width / 2) + 2,
					y: node.y + ((0.5+(i+.5)*config.leading)*config.fontSize) - (compartment.height / 2) + 25
				}
			}
		};
	}
	...
	function renderRelation(r, compartment){
		...
		var start = findPropertyCoordinates(startNode, r.startProperty);
		var end = findPropertyCoordinates(endNode, r.endProperty);
		if (typeof(end) == 'undefined') {
			var end = rectIntersection(r.path[r.path.length-2], _.last(r.path), endNode)
		}
		// console.log(endNode, r.endProperty, end);
		...
		g.ctx.beginPath();
		g.ctx.arc(start.x, start.y, 5, 0, 2 * Math.PI, false);
		g.ctx.fillStyle = config.stroke;
		g.ctx.fill();
		g.ctx.lineWidth = 2;
